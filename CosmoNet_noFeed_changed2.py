...
### to move to hyper_parameters.py
zscore_average_constant = [2.995679839999998983e-01,8.610806619999996636e-01]
zscore_std_constant = [2.905168635566176411e-02,4.023372385668218254e-02]
...
#            h_conv6 = lrelu(self.BatchNorm(tf.nn.conv3d(h_conv5, self.W['W_conv6'],strides = [1,1,1,1,1],padding = 'VALID') + self.b['b_conv6'],IS_TRAINING=IS_TRAINING,scope = scope+str(6),reuse=reuse),hp.Model['LEAK_PARAMETER'])
    ...
    def validation_loss(self):
        ...
        val_predict = val_predict*tf.constant(zscore_std_constant,dtype = tf.float32)+tf.constant(zscore_average_constant,dtype = tf.float32)
        val_true = self.val_label*tf.constant(zscore_std_constant,dtype = tf.float32)+tf.constant(zscore_average_constant,dtype = tf.float32)
    ...
    def train_loss(self):
        ...
        train_predict = train_predict*tf.constant(zscore_std_constant,dtype = tf.float32)+tf.constant(zscore_average_constant,dtype = tf.float32)
        train_true = self.train_label*tf.constant(zscore_std_constant,dtype = tf.float32)+tf.constant(zscore_average_constant,dtype = tf.float32)
    ...
    def test_loss(self):
        ...
        test_predict = test_predict*tf.constant(zscore_std_constant,dtype = tf.float32)+tf.constant(zscore_average_constant,dtype = tf.float32)
        test_true = self.test_label*tf.constant(zscore_std_constant,dtype = tf.float32)+tf.constant(zscore_average_constant,dtype = tf.float32)
